...
<style>
    ...
    /* Abstract placeholder if no image exists */
    .h-visual-orb {
        position: absolute;
        width: 70%;
        height: 70%;
        border-radius: 50%;
        background: linear-gradient(135deg, rgba(255,60,0,0.8), rgba(200,241,53,0.2));
        filter: blur(20px);
        animation: pulseOrb 6s alternate infinite ease-in-out;
    }
...
<section class="h-hero-split" aria-labelledby="h-headline">
    ...
    <div class="h-hero-right" aria-hidden="true">
        <div class="h-visual-wrap">
            <div class="h-visual-orb"></div>
            <div class="h-visual-ring"></div>
            </div>
    </div>
</section>
...
<section class="h-svc" aria-labelledby="h-svc-h">
    ...
    <div class="h-svc-grid rv">
        <?php $svcs = [
            ...
            ['08', 'Performance Marketing', 'Data-driven paid campaigns that amplify organic wins and turn attention into revenue.'],
            ...
            ['11', 'On-Ground Promotion', 'Physical brand activations anchored in digital culture. The moment that becomes the post.'],
        ];
        foreach ($svcs as $s): ?>
...
// Create arrays for Row 1 and Row 2
$items = !empty($ticker_imgs) ? $ticker_imgs : $fallback_brands;
$is_img = !empty($ticker_imgs);
$row1_items = array_merge($items, $items, $items);
$row2_items = array_reverse($row1_items);
?>
